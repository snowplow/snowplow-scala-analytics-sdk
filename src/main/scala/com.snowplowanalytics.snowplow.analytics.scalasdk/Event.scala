...
object Event {
   ...
   */
  private lazy val parser: Parser[Event] = Parser.deriveFor[Event].get(Map.empty)

  /**
   * Derived TSV parser for the Event class
   */
  private lazy val parserWithValidation: Parser[Event] = Parser.deriveFor[Event].get(FIELD_SIZES)

  /**
   * Converts a string with an enriched event TSV to an Event instance,
   * or a ValidatedNel containing information about errors
   *
   * `validateFieldLengths` can be `false` in most circumstances, because Enrich already performs
   * this validation since version 3.0.0. Toggle it to `true` if atomic field lengths are important
   * to you, and either you are parsing events generated by an older version of enrich, or if you
   * run enrich with the `featureFlags.acceptInvalid` config option on.
   *
   * @param line Enriched event TSV line
   * @param validateFieldLengths Whether to validate that field lengths comply to the atomic schema from Iglu Central
   */
  def parse(line: String, validateFieldLengths: Boolean): DecodeResult[Event] =
    if (validateFieldLengths)
      parserWithValidation.parse(line)
    else
      parser.parse(line)
   ...
   * or a ValidatedNel containing information about errors
   *
   * Equivalent to `Event.parse(line, validateFieldLengths = false)`
   *
   * @param line Enriched event TSV line
   */
  def parse(line: String): DecodeResult[Event] =
    parse(line, validateFieldLengths = false)
