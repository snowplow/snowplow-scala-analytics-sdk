...
import com.snowplowanalytics.snowplow.analytics.scalasdk.decode.{Parser, RowDecodeResult}
...
case class Event(app_id:                   Option[String],
  ...
  /**
    * @return the metadata about where an event was extracted from
    */
  ...
  /**
    * @return the Event, represented as a map of keys to Circe JSON values
    */
  private def toJsonMap: Map[String, Json] = this.asJsonObject.toMap

  /**
    * @return the Event, represented as a map of keys to Circe JSON values without inventory fields
    */
  def atomic: Map[String, Json] = toJsonMap - "contexts" - "unstruct_event" - "derived_contexts"

  /**
    * @return the Event, represented as a list of key/JSON value pairs
    *         using the ordering of the canonical event model
    */
  def ordered: List[(String, Option[Json])] =
    Event.parser.knownKeys.map(key => (key.name, toJsonMap.get(key.name)))

  /**
    * @return a compound JSON field containing latitude/longitude information,
  ...
  /**
    * @return either a simple JSON mapping of an event
    *         or a lossy mapping with transformed Elasticsearch-compatible contexts and unstructured_event fields,
    *         as well as an additional geo_location field
    */
  def toJson(lossy: Boolean): Json =
    if (lossy) {
      JsonObject.fromMap(atomic ++ contexts.toJson.toMap ++ derived_contexts.toJson.toMap ++ unstruct_event.toJson.toMap ++ geoLocation).asJson
    } else {
      this.asJson
    }
}
...
object Event {
  ...
  def parse(line: String): RowDecodeResult[Event] =
    parser.parse(line)
}
